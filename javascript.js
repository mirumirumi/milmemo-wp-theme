...
function writeLikeButtonDB(type, url) {
    $.ajax({
        type: "POST",
        url: "https://milmemo.net/wp-admin/admin-ajax.php",
        data: {
            "action": "like_button",
            "type": type,
            "slug": url
...
$(document).ready(function () {
    ...
    let tag = '<div class="secret-password">'+ passwordSet[randomPickArray] + '<a href="https://milmemo.net/what-is-this-blog#omake">' + String.fromCodePoint(0x3053) + String.fromCodePoint(0x308C) + String.fromCodePoint(0x306F) + String.fromCodePoint(0x4F55) + String.fromCodePoint(0xFF1F) + '</a></div>';

    if (randomDispPage === 0 //確率7.1%
        && window.performance.navigation.type === 0 //リンクのクリックのみ
        && location.href !== "https://milmemo.net/"
        && url !== "all-entries"
        && url !== "profile"
...
//*********************画像拡大可能アイコンを表示する******************
// $(document).ready(function () {
//     let imgTag = $(".entry-content").find("img");
//     let imgTagParent = imgTag.parent();
//     $.each(imgTagParent, function(index, value) {
//         if ($(value).is("a")) {
//             $(value).parent().addClass("imgImpZoom");
//         }
//     });
// });
