...
<article id="post-<?php the_ID(); ?>" <?php post_class('article') ?> itemscope="itemscope" itemprop="blogPost" itemtype="https://schema.org/BlogPosting">
  ...
  if ( have_posts() ) {
    ...
    <?php if(!is_page(12717)) { //トップページではタイトルなどを非表示 ?>
      ...
      <header class="article-header entry-header">
        ...
        */ ?>
        ...
        <?php //投稿日と更新日テンプレート （デフォルトとアイキャッチなどと順番は変えてる）
        ...
        <?php /* if (is_content_read_time_visible() && is_the_page_read_time_visible() && !is_plugin_fourm_page()): ?>
          <div class="read-time"><?php echo '<span class="fa fa-hourglass-half" aria-hidden="true"></span>'.sprintf(__( 'この記事は<span class="bold">約%s分</span>で読めます。', THEME_NAME ), get_time_to_content_read(get_the_content())); ?></div>
        <?php endif; */ ?>
        ...
        <?php /*
            もともとSNS直下ビッグバナーがいた。再開する場合は、他と同じようにPCもモバイルも統一させて１つのユニットを使うこと！
        */ ?>

        <?php //プロフィールボックスをカテゴリによって読み分けるファイルを外部化
        if(!is_page())
            get_template_part("profile_top");
        ?>
    ...
    <?php } //トップページではタイトルなどを非表示 ?>
      ...
      <footer class="article-footer entry-footer">
        ...
        <?php //プロフィールボックスをカテゴリによって読み分けるファイルを外部化
        if(!is_page())
            get_template_part("profile_bottom");
        ?>
        ...
        <?php //みるめも_記事下_ディスプレイ広告(スクエア)
              //テスト完了したので戻ってきた（2021/2/22~）?>
        <?php $is_disp = apply_filters('is_adsense_disp',$is_disp); 
        ...
        <div class="publisher" itemprop="publisher" itemscope itemtype="https://schema.org/Organization">
            <div itemprop="logo" itemscope itemtype="https://schema.org/ImageObject">
              <img loading="lazy" src="<?php echo $home_image_url; ?>" width="<?php echo $width; ?>" height="<?php echo $height; ?>" alt="">
